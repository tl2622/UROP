...
xi_1 = np.zeros_like(x)
xi_1[(x >= np.pi/2) & (x <= 3*np.pi/2)] = 0.1
...
for n in range(0, M-1):
    ...
    if n % 10 == 0:
        ...
        plt.ylabel('u(x)')
        plt.ylim(-1.5, 1.5)
        plt.title('Stochastic Burgers Equation with Transport Noise --- square case')
        plt.legend()
...
clip.write_videofile("stochastic_burgers_transport_noise_square.mp4", codec="libx264")
